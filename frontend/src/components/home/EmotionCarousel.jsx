...
export default function EmotionCarousel() {
  ...
  return (
    <Box position="relative" w="100%" h="100%" minH="188px" maxW="100%" ml={0} borderRadius="0" overflow="hidden" className="vl-cid-home-carousel" data-cid="home-carousel">
      {/* Scroll track */}
      <HStack
        ref={trackRef} h="100%"
        spacing={0}
            ...
            {(s[lang]?.title || s[lang]?.cta) && (
              <>
                ...
                <VStack position="absolute" bottom={0} left={0} right={0} p={[4,6]} align="flex-start" spacing={3} color="white">
                  ...
                  {s[lang]?.cta && (
                    <HStack>
                      <Button as={RouterLink} to="/chat" size="sm" colorScheme="blue" bgGradient="var(--vl-gradient-cta)" _hover={{ bgGradient: 'var(--vl-gradient-cta-hover)', transform: 'translateY(-1px)' }} borderRadius="full">
                        {s[lang].cta}
                      </Button>
                    </HStack>
                  )}
      ...
      <IconButton aria-label="Previous" icon={<FaChevronLeft />} onClick={prev}
                  ...
                  bg="blackAlpha.500" color="white" _hover={{ bg: 'blackAlpha.700' }} data-cid="home-carousel-prev" className="vl-cid-home-carousel-prev" />
      <IconButton aria-label="Next" icon={<FaChevronRight />} onClick={next}
                  ...
                  bg="blackAlpha.500" color="white" _hover={{ bg: 'blackAlpha.700' }} data-cid="home-carousel-next" className="vl-cid-home-carousel-next" />

      {/* Dots */}
      <HStack position="absolute" bottom={3} left="50%" transform="translateX(-50%)" spacing={2} className="vl-cid-home-carousel-dots" data-cid="home-carousel-dots">
        {slides.map((_, i) => (
          <Box key={i} as="button" onClick={() => scrollTo(i)} w={i === index ? 6 : 2} h={2} borderRadius="full" bg={i === index ? 'white' : 'whiteAlpha.700'} aria-label={`Go to slide ${i+1}`} data-cid={`home-carousel-dot-${i}`} className="vl-cid-home-carousel-dot">
